...
#             if isinstance(item, type) and issubclass(item, Command) and item is not Command:
#                 # Command names are now explicitly set to the plugin's folder name
#                 self.command_handler.add_command(plugin_name, item())
#                 logging.info(f"Command '{plugin_name}' from plugin '{plugin_name}' registered.")

#     def start(self):
#         self.load_plugins()
#         logging.info("Application started.")
#         print(f"\nAvailable commands are: {', '.join(list(self.command_handler.commands.keys()))}")
#         print(f"Usage: Command num1 num2 (Ex: add 3 4) or type 'exit' to exit.\n")
        
#         try:
#             while True:  #REPL Read, Evaluate, Print, Loop
#                 inp_text = input(">>> ").strip().split(sep=" ")
#                 self.command_handler.add_command(inp_text[0], inp_text[1:])
